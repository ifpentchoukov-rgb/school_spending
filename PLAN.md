...
- [x] **7.5 — Universal floor pass (complete 2026-05-15).** Worked through all 26 moderate-tier states. 11 promoted to rich tier along the way (net +11 rich states, so 19 rich total). 5 sources are inherently topline-only and skipped for component emission (FL Section II PDF, MA DESE PPX HTML, TN ASR Table 51, ND FinFacts PDF, MO DESE MCDS Finance Summary).
    ...
    **Batch 1 (9 states, 13,959 components):**
    ...
    - **OR → rich**: 179 × 9 = **1,573** (ODE Detailed District Expenditure — function-code bucketing)
    - **OK → rich**: 428 × 8 = **3,412** (OSDE OCAS — function+object bucketing; capital+debt excluded by source filter)
...
- [x] **8.1 — Schema** (migration 0012). Applied 2026-05-15. `ALTER TABLE districts ADD COLUMN entity_type text NOT NULL DEFAULT 'district' CHECK (entity_type IN ('district', 'cooperative', 'charter'))`. New `cooperative_membership(cooperative_leaid, member_leaid, fiscal_year, allocation_share, allocation_basis)` with composite PK + indexes on each side. Anon-read RLS; service-role writes. All 11,880 existing districts default to `entity_type='district'`.
- [ ] **8.2 — NY BOCES extractor** (`extractors/ny_boces.py`). **Blocked on source discovery (2026-05-15).** Original plan assumed 37 BOCES file in the same SAMS XLSX as districts — confirmed wrong: `https://stateaid.nysed.gov/st3/st3data.htm` is districts-only (698 entries, 0 with 'BOCES' in name). NYSED's `/boces/` page lists training manuals + state-aid output reports but no bulk financial feed. Path forward (multi-session): (a) per-BOCES PDF scrape from each BOCES's annual report page, (b) FOIA NYSED Office of State Aid for a SAMS BOCES extract, (c) NYSED SAMS API access if available to authorized users. Defer until source path chosen.
- [ ] **8.3 — NJ ESC, RI regional, CT regional extractors.** Same scoping question — need to verify whether NJ TGES file includes ESC rows (231/265 matched suggests 34 unmatched might be ESCs/regionals), whether RI Tableau dashboards include regional districts, and whether CT OPM SODA API includes regional districts. Defer alongside 8.2; smaller scale (~27 total entities) but same investigation pattern.
- [x] **8.4 — `v_district_full_spend` view** (migration 0013). Applied 2026-05-15. Returns one row per `(member_leaid, fiscal_year, status)` with `own_spend`, `allocated_cooperative_spend`, `total_full_spend`, `full_spend_per_pupil`. Allocation = `SUM(coop_topline × allocation_share)` joined via `cooperative_membership`. Filters out cooperatives themselves (`entity_type='district'` predicate). Works correctly today returning 0 allocation everywhere (cooperative_membership is empty); flips on automatically once 8.2/8.3 populate the join table. Verified: 13,312 rows (same universe as `v_per_pupil_metrics`), 0 with allocation (expected).

**Acceptance (deferred):** NY districts that belong to a BOCES will show non-zero `allocated_cooperative_spend` in `v_district_full_spend` once 8.2 populates `cooperative_membership`; spot-checks against published BOCES annual reports should match within ±2%.
...
- [x] **9.1 — Search & filter UX** (web commit `e3e567c`, 2026-05-15). Sticky top nav header with backdrop blur; inline search input visible md+, `/search` nav link on mobile. New `/search?q=&state=&page=` route — ILIKE on `lea_name` across 11,880 operating districts, sorted by enrollment desc, 50/page. Strips %/_ wildcards and caps query length at 80 chars. Autocomplete deferred (would require client-side JS).
- [x] **9.2 — Rankings** (`63c5efe`, 2026-05-15). `/rankings?metric=&state=&band=&fy=&order=&page=` over `v_per_pupil_metrics`. 8 metrics (total/instruction/admin/debt/capital/benefits/food per-pupil + yoy_change_pct), state filter, enrollment band (mega/large/mid/small), order toggle. Auto-resolves latest available FY scoped to the current state filter. Paginated 50/page; tier badge per row (rich/moderate/thin/deferred from `state_extractor_metadata`) with `tier_rationale` tooltip.
- [x] **9.3 — Compare side-by-side** (`7bcda7d`, 2026-05-15). `/compare?leaids=A,B,C,D` (max 4). Rows = 12 canonical expenditure categories + 3 revenue groupings, cells = per-pupil dollars. Max-value-per-row cell tinted. Per-comparison CSV at `/api/compare/csv?leaids=`. Time-series sparkline per category was scoped out for v1 (static comparison covers the common case; sparklines would 4× the data fetch).
- [x] **9.4 — Per-LEA enrichment** (`2ba583f`, 2026-05-15). `CategoryBreakdown` component on the per-LEA page: rows = canonical expenditure_category enum values, cols = up to 4 most-recent actual FYs, rightmost column = per-pupil for latest FY. Coverage gaps render as faded em-dash. `CoverageTierBadge` in the header with `tier_rationale` tooltip linking to methodology. Peer cohort builder + cooperative attribution deferred (cooperatives blocked by Phase 8.2 source discovery).
- [~] **9.5 — Mobile + accessibility.** Responsive scaffolding shipped across the new pages — sticky header with backdrop blur, `grid-cols-1 sm:grid-cols-*` for cards, `overflow-x-auto` on wide tables, `aria-label`s on search/filter controls, semantic `<th scope="row">` in pivot tables. Dedicated mobile-card-view refactor of the comparison + rankings tables is a separate UX project, deferred. Programmatic WCAG AA contrast audit also deferred — spot-checked Tailwind slate/sky defaults pass on body text; tertiary meta text uses `slate-400/500` (just below AA on white) which is the intentional de-emphasis cue.
- [x] **9.6 — Public CSV downloads** (`a437d4b`, 2026-05-15). Three CSV endpoints: `/api/compare/csv` (per-comparison, Phase 9.3); `/api/exports/state/{postal}/csv?fy=&status=` (39-column full `v_per_pupil_metrics` slice per state); `/api/exports/rankings/csv?metric=&state=&band=&fy=&order=` (mirrors `/rankings` filters, rank column, 5000-row cap). All return `text/csv` with `Content-Disposition: attachment` and a 1-hour edge cache. Download buttons wired into per-state page, rankings filter bar, and compare page. Edge rate limiting deferred to Phase 10.2 (researcher tier work).
...
- [~] **11.1 — Collect FY27 adopted budgets.** Passive — happens via existing extractor cron + on-demand `extractor_triggers` table. As of 2026-05-15: only HI has FY27 ($2.86B, biennial via Act 88/2025). NJ adoption deadline today; ~10 adopted-budget pipelines (TX/FL/CA/PA/WA/IN/NJ/CT/KS/HI) will populate `(fiscal_year=2027, status='adopted')` rows as each state closes its window. Expected curve: ~600 districts by Jun 30 (NJ + early NY referenda), ~3,000 by end of August (most Jun-30 states settled), ~11k by Nov (PA/CA/TX/WA/IN/CT/KS late closers). No new pipeline code needed; existing extractors already advance fiscal_year on the next run.
- [x] **11.2 — `v_fy27_rollup` view** (migration 0014, 2026-05-15). Per-LEA join of FY27 adopted vs FY26 baseline (DISTINCT ON-prefers FY26 actual, falls back to FY26 adopted; baseline status exposed in `fy26_baseline_status`). Exposes `fy27_amount`, `fy26_baseline_amount`, `dollar_change`, `pct_change`, `fy27_per_pupil`, `fy26_per_pupil`, plus a pre-computed `change_bucket` (`lt_neg_10pct` / `neg_10_to_neg_5pct` / `neg_5_to_0pct` / `0_to_5pct` / `5_to_10pct` / `gte_10pct` / `no_baseline`) so the report-page histogram doesn't re-do the math. Filters out cooperatives via `entity_type='district'`. Category-level deltas (§7 components) deferred — they'd 14× the row width and the topline delta is sufficient for the public report.
- [x] **11.3 — Published artifact at `/reports/fy27`** (web commit `45fb863`, 2026-05-15). 4 topline cards (districts reporting / FY27 sum / FY26 baseline sum / aggregate %); 6-bucket YoY histogram (only over rows with a baseline, normalized to max bucket); per-state aggregate table (districts, FY27, FY26, median %); top-10 gainers + decliners with enrollment ≥1,000 floor; methodology callout enumerating baseline preference, frame definitions, and coverage gaps (NYC DOE, charters, 6 deferred states); press-kit section linking the three bulk CSVs. Auto-refreshes hourly (revalidate=3600); empty-state path shows upcoming state adoption deadlines from `state_calendars`.
- [x] **11.4 — Press kit** (web commit `45fb863`). New `/api/v1/exports/fy27-rollup.csv?state=` endpoint serving the full 20-column rollup (both FY columns + delta + change_bucket + event/source IDs for cross-referencing). The methodology PDF is implicit in the `/methodology/[postal]` pages (Phase 10.4) — each state's deep-link is one click from the FY27 report. Standalone PDF generation deferred (would require a headless-Chrome render service for the data viz; the methodology pages themselves are already print-friendly).

**Acceptance:** `/reports/fy27` renders today with HI as the lone reporting district. Once FY27 collection accelerates over June–August 2026, the page will populate automatically: histogram, state table, top-10 movers all derive from `v_fy27_rollup`, which derives from `budget_events`, which the existing extractor pipeline writes to. Manual press-kit publication still wanted ~Sept 2026 once coverage reaches ≥90% of US enrollment.
